...
async def my_unmute(guild: discord.Guild, member: discord.Member, end_time, offence_id) -> None:
    ...
    if end_time is None:
        ...
        Globals.conn.commit()
    if msg_needed:
        embed = discord.Embed(title=f"you have been unmuted from {guild.name}", colour=0x0011ff,
...
class Moderation(commands.Cog):
    ...
    async def tempban(self, ctx: commands.Context, user: discord.User, time: str, *args):
        ...
        try:
            ...
            embed = discord.Embed(description=f"offence id: {offence_data[0]}",
    ...
    async def unban(self, ctx: commands.Context, user_id: int):
        ...
        for ban_entry in all_bans:
            ...
            if user.id == ban_entry.user.id:
                ...
                Globals.conn.commit()
                if data:
                    await my_unban(ctx.guild, user, data[0])
                else:
    ...
    async def ban(self, ctx: commands.Context, user: discord.User, *args):
        if moderationEnabled(ctx.guild):
            ...
            c.execute("""insert into offences(member_id,guild_id,kind,start_date,reason,treated,moderator_id)
                        values(?,?,?,?,?,?,?)""",
                      (user.id, ctx.guild.id, "tempban",
                       datetime.datetime.utcnow(),
                       reason, "false", ctx.author.id))
            Globals.conn.commit()
            c.execute("""select * from offences
            where guild_id=:guild_id and start_date=:start_date and kind=:kind and member_id=:member_id""",
                      {"guild_id": ctx.guild.id, "start_date": start_time, "kind": "tempban", "member_id": user.id})
            offence_data = c.fetchone()
            embed = discord.Embed(title=f"{user} has been baned", description=f"offence id: {offence_data[0]}",
                                  timestamp=datetime.datetime.utcnow(), colour=0xe74c3c)
            embed.add_field(name="reason:", value=reason)
            try:
                await user.send(embed=embed)
    ...
    async def mute(self, ctx: commands.Context, member: discord.Member, *args):
        if moderationEnabled(ctx.guild):
            ...
            Globals.conn.commit()
            c.execute("""select * from offences
            where guild_id=:guild_id and member_id=:member_id and start_date=:start_date and kind=:kind""",
                      {"guild_id": ctx.guild.id, "member_id": member.id, "start_date": start_date, "kind": "tempmute"})
            offence_data = c.fetchone()
            embed = discord.Embed(title=f"{member} has been muted", description=f"offence id: {offence_data[0]}",
                                  timestamp=datetime.datetime.utcnow(),
                                  colour=0xe74c3c)
            embed.add_field(name="reason:", value=reason)
            await ctx.send(embed=embed)
    ...
    async def tempmute(self, ctx: commands.Context, member: discord.Member, time: str, *args):
        if moderationEnabled(ctx.guild):
            ...
            try:
                ...
                c = Globals.conn.cursor()
                c.execute("""insert into offences(member_id,guild_id,kind,start_date,end_date,reason,treated,moderator_id)
                        values(?,?,?,?,?,?,?,?)""", (member.id, ctx.guild.id, "tempmute", start_date,
                                                     start_date + timedelta(seconds=time_by_seconds),
                                                     reason,
                                                     "false", ctx.author.id))
                Globals.conn.commit()
                c.execute("""select * from offences
                where guild_id=:guild_id and member_id=:member_id and start_date=:start_date and kind=:kind and end_date=:end_date""",
                          {"guild_id": ctx.guild.id, "member_id": member.id, "start_date": start_date,
                           "kind": "tempmute", "end_date": start_date + timedelta(seconds=time_by_seconds)})
                offence_data = c.fetchone()
                embed = discord.Embed(
                    title=f"{member} has been muted until {datetime.datetime.utcnow() + timedelta(seconds=time_by_seconds)}",
                    timestamp=datetime.datetime.utcnow(), description=f"offence id: {offence_data[0]}", colour=0xe74c3c)
                embed.add_field(name="reason", value=reason)
                await ctx.send(embed=embed)
                try:
                    await member.send(embed=embed)
    ...
    async def warn(self, ctx: commands.Context, member: discord.Member, *args):
        if ctx.author.guild_permissions.mute_members or is_a_moderator(ctx.author):
            if moderationEnabled(ctx.guild):
                ...
                Globals.conn.commit()
                c.execute("""select * from offences
                          WHERE guild_id = :guild_id and member_id = :member_id and kind = :kind and start_date = :start_date""",
                          {"guild_id": ctx.guild.id, "member_id": member.id, "kind": "warn", "start_date": time})
                offence_data = c.fetchone()
                embed = discord.Embed(title=f"{member} **has been warned**",
                                      description=f"offence id: {offence_data[0]}", timestamp=time)
    ...
    async def kick(self, ctx: commands.Context, member: discord.Member, *args):
        if moderationEnabled(ctx.guild):
            ...
            c.execute("""select * from offences
                      where guild_id=:guild_id and member_id=:member_id and kind=:kind and start_date=:start_date""",
                      {"guild_id": ctx.guild.id, "member_id": member.id, "kind": "kick", "start_date": time})
            offence_data = c.fetchone()
            embed = discord.Embed(title=f"{member} has been kicked", description=f"offense id: {offence_data[0]}",
                                  timestamp=time,
                                  colour=0xe74c3c)
            embed.add_field(name="reason:", value=reason)
            try:
    ...
    async def offence(self, ctx: commands.Context, offence_id: int):
        if ctx.author.guild_permissions.mute_members or ctx.author.guild_permissions.kick_members or ctx.author.guild_permissions.mute_members or ctx.author.guild_permissions.ban_members or is_a_moderator(
            ...
            if moderationEnabled(ctx.guild):
                c = Globals.conn.cursor()
                c.execute("""select * from offences
                where offence_id = :offence_id and guild_id=:guild_id""",
                          {"offence_id": offence_id, "guild_id": ctx.guild.id})
                offence_data = c.fetchone()
                if not offence_data:
