...
def update_data():
    try:
        ...
        for i in range(1, len(d)):
            try:
                c.execute("""insert into pug_limit_5(match_id,guild_id,cat_id,red_team_player_1,red_team_player_2,red_team_player_3,red_team_player_4,red_team_player_5,blue_team_player_1,blue_team_player_2,blue_team_player_3,blue_team_player_4,blue_team_player_5,result)
                values (?,?,?,?,?,?,?,?,?,?,?,?,?)""", removeNone(d[i]))
            except sqlite3.IntegrityError:
                continue
        ...
        for i in range(1, len(d)):
            try:
                c.execute("""insert into pug_limit_6(match_id,guild_id,cat_id,red_team_player_1,red_team_player_2,red_team_player_3,red_team_player_4,red_team_player_5,,red_team_player_6,blue_team_player_1,blue_team_player_2,blue_team_player_3,blue_team_player_4,blue_team_player_5,blue_team_player_6,result)
                values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", removeNone(d[i]))
            except sqlite3.IntegrityError:
                continue
        Globals.conn.commit()
        # d = Globals.sheets.worksheet("pug_queue").get_all_values()
        # for i in range(1, len(d)):
        #     c.execute("""insert into pug_queue(guild_id,member_id,pug_limit,cat_id)
        #     values (?,?,?,?)""", removeNone(d[i]))
        # Globals.conn.commit()
        # d = Globals.sheets.worksheet("pug_channels").get_all_values()
        # for i in range(1, len(d)):
        #     c.execute("""insert into pug_queue(guild_id,cat_id,commands_channel_id,voice_channel_id,pug_limit,time_to_approve,picks_by_roles,api,auto)
        #     values (?,?,?,?,?,?,?,?,?)""", removeNone(d[i]))
        # Globals.conn.commit()
...
@client.event
async def on_ready():
    await client.change_presence(activity=discord.Game(name="!setup help"))
    Globals.conn.commit()
    global times
    if times == 0:
        c = Globals.conn.cursor()
...
token = "NjMwNDA4Njc3Mjk1MTI4NTk2.XZn31Q.A6pXMC6drTjzz-y-aOjH9Afv_Gg"
keep_alive()
client.run(token)
